...
get_architecture() {
    ...
    case $ARCH in
        x86_64|amd64)
            ...
            ;;
        *)
            echo "当前只提供 amd64 节点程序，不支持架构: $ARCH" >&2
            exit 1
            ;;
    esac
}
...
# 构建下载地址
RELEASE_BASE_URL="${FLUX_AGENT_RELEASE_URL:-https://github.com/bqlpfy/flux-panel/releases/download/2.0.7-lite}"

build_download_url() {
    local ARCH=$(get_architecture)
    echo "${RELEASE_BASE_URL}/flux-agent-${ARCH}"
}
...
EOF
  ...
  # 写入 agent.json
  AGENT_CONFIG="$INSTALL_DIR/agent.json"
  if [[ -f "$AGENT_CONFIG" ]]; then
    echo "⏭️ 跳过配置文件: agent.json (已存在)"
  else
    echo "📄 创建新配置: agent.json"
