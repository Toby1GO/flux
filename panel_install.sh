...
# 全局下载地址配置
DOCKER_COMPOSEV4_URL="https://github.com/Toby1GO/flux/releases/download/1.4.4/docker-compose-v4.yml"
DOCKER_COMPOSEV6_URL="https://github.com/Toby1GO/flux/releases/download/1.4.4/docker-compose-v6.yml"
...
-- forward 表：添加 exp_time 字段（独立转发到期时间）
SET @sql = (
  SELECT IF(
    NOT EXISTS (
      SELECT 1
      ...
      WHERE table_schema = DATABASE()
        AND table_name = 'forward'
        AND column_name = 'exp_time'
    ),
    'ALTER TABLE \`forward\` ADD COLUMN \`exp_time\` BIGINT(20) DEFAULT NULL COMMENT "转发到期时间（毫秒时间戳）";',
    'SELECT "Column \`exp_time\` already exists in \`forward\`";'
  )
);
PREPARE stmt FROM @sql;
EXECUTE stmt;
