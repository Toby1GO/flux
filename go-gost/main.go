...
func worker(id int, args []string, ctx *context.Context, ret *int) {
	...
	cmd.Env = append(os.Environ(), fmt.Sprintf("_FLUX_AGENT_ID=%d", id))
...
func init() {
	...
	if printVersion {
		fmt.Fprintf(os.Stdout, "flux-agent %s (%s %s/%s)\n",
...
// GOOS=linux GOARCH=amd64 go build -ldflags="-s -w" -o flux-agent
// upx --best --lzma flux-agent
